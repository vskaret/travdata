-- bruker 0.3 som vekt
CREATE MATERIALIZED VIEW gwtimeweight30 AS
SELECT
  s.dato, s.tidspunkt, s.travbane, s.lopnr, s.hest,
  g.dato ldato, g.travbane lbane, g.lopnr lnr,
  CASE WHEN ((s.dato - g.dato)::NUMERIC / 365) <= 1 THEN 0.3 ELSE ROUND(0.3^((s.dato - g.dato)::NUMERIC / 365), 6) END tweight,
...
CREATE MATERIALIZED VIEW weightedg30 AS
...
CREATE MATERIALIZED VIEW antstarterwtimeweight30 AS
SELECT f.dato, f.tidspunkt, f.travbane, f.lopnr, f.hest,
       l.dato ldato, l.travbane lbane, l.lopnr lnr,
       CASE WHEN ((f.dato - l.dato)::NUMERIC / 365) <= 1 THEN 0.3 ELSE ROUND(0.3^((f.dato - l.dato)::NUMERIC / 365), 6) END tweight,
       1 AS start
FROM starter f
...
SELECT d.dato, d.tidspunkt, d.travbane, d.lopnr, d.hest, d.snr, d.rase, d.hdist, d.km okm,
       l.dato ldato, l.travbane lbane, l.lopnr lnr,
       CASE WHEN ((d.dato - l.dato)::NUMERIC / 365) <= 1 THEN 0.3 ELSE ROUND(0.3^((d.dato - l.dato)::NUMERIC / 365), 6) END tweight,
       l.km, l.premie
FROM dnf d
...
CREATE MATERIALIZED VIEW weightedkm30 AS
...
-- for csv
CREATE MATERIALIZED VIEW h2hv30 AS
...
drop materialized view h2hv30, wkmpluswg30, weightedkm30, kmtimeweight30, weightedstarter30, antstarterwtimeweight30, weightedg30, gwtimeweight30;
