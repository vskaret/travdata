-- bruker 0.5 som vekt
CREATE MATERIALIZED VIEW gwtimeweight50 AS
SELECT
  s.dato, s.tidspunkt, s.travbane, s.lopnr, s.hest,
  g.dato ldato, g.travbane lbane, g.lopnr lnr,
  CASE WHEN ((s.dato - g.dato)::NUMERIC / 365) <= 1 THEN 0.5 ELSE ROUND(0.5^((s.dato - g.dato)::NUMERIC / 365), 6) END tweight,
...
CREATE MATERIALIZED VIEW weightedg50 AS
...
CREATE MATERIALIZED VIEW antstarterwtimeweight50 AS
SELECT f.dato, f.tidspunkt, f.travbane, f.lopnr, f.hest,
       l.dato ldato, l.travbane lbane, l.lopnr lnr,
       CASE WHEN ((f.dato - l.dato)::NUMERIC / 365) <= 1 THEN 0.5 ELSE ROUND(0.5^((f.dato - l.dato)::NUMERIC / 365), 6) END tweight,
       1 AS start
FROM starter f
...
SELECT d.dato, d.tidspunkt, d.travbane, d.lopnr, d.hest, d.snr, d.rase, d.hdist, d.km okm,
       l.dato ldato, l.travbane lbane, l.lopnr lnr,
       CASE WHEN ((d.dato - l.dato)::NUMERIC / 365) <= 1 THEN 0.5 ELSE ROUND(0.5^((d.dato - l.dato)::NUMERIC / 365), 6) END tweight,
       l.km, l.premie
FROM dnf d
...
CREATE MATERIALIZED VIEW weightedkm50 AS
...
-- for csv
CREATE MATERIALIZED VIEW h2hv50 AS
...
TO '/var/lib/pgsql/midl/h2hv100.csv' DELIMITER ';';

drop materialized view h2hv50, wkmpluswg50, weightedkm50, kmtimeweight50, weightedstarter50, antstarterwtimeweight50, weightedg50, gwtimeweight50;
