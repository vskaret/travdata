-- bruker 0.9 som vekt
CREATE MATERIALIZED VIEW gwtimeweight90 AS
SELECT
  s.dato, s.tidspunkt, s.travbane, s.lopnr, s.hest,
  g.dato ldato, g.travbane lbane, g.lopnr lnr,
  CASE WHEN ((s.dato - g.dato)::NUMERIC / 365) <= 1 THEN 0.9 ELSE ROUND(0.9^((s.dato - g.dato)::NUMERIC / 365), 6) END tweight,
...
CREATE MATERIALIZED VIEW weightedg90 AS
...
CREATE MATERIALIZED VIEW antstarterwtimeweight90 AS
SELECT f.dato, f.tidspunkt, f.travbane, f.lopnr, f.hest,
       l.dato ldato, l.travbane lbane, l.lopnr lnr,
       CASE WHEN ((f.dato - l.dato)::NUMERIC / 365) <= 1 THEN 0.9 ELSE ROUND(0.9^((f.dato - l.dato)::NUMERIC / 365), 6) END tweight,
       1 AS start
FROM starter f
...
SELECT d.dato, d.tidspunkt, d.travbane, d.lopnr, d.hest, d.snr, d.rase, d.hdist, d.km okm,
       l.dato ldato, l.travbane lbane, l.lopnr lnr,
       CASE WHEN ((d.dato - l.dato)::NUMERIC / 365) <= 1 THEN 0.9 ELSE ROUND(0.9^((d.dato - l.dato)::NUMERIC / 365), 6) END tweight,
       l.km, l.premie
FROM dnf d
...
CREATE MATERIALIZED VIEW weightedkm90 AS
...
-- for csv
CREATE MATERIALIZED VIEW h2hv90 AS
...
drop materialized view h2hv90, wkmpluswg90, weightedkm90, kmtimeweight90, weightedstarter90, antstarterwtimeweight90, weightedg90, gwtimeweight90;
