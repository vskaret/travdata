...
SELECT
  ...
  CASE WHEN ((s.dato - g.dato)::NUMERIC / 365) <= 1 THEN 0.4 ELSE ROUND(0.4^((s.dato - g.dato)::NUMERIC / 365), 6) END tweight,
...
CREATE MATERIALIZED VIEW antstarterwtimeweight40 AS
SELECT f.dato, f.tidspunkt, f.travbane, f.lopnr, f.hest,
       l.dato ldato, l.travbane lbane, l.lopnr lnr,
       CASE WHEN ((f.dato - l.dato)::NUMERIC / 365) <= 1 THEN 0.4 ELSE ROUND(0.4^((f.dato - l.dato)::NUMERIC / 365), 6) END tweight,
       1 AS start
FROM starter f
...
CREATE MATERIALIZED VIEW weightedstarter40 AS
SELECT dato, tidspunkt, travbane, lopnr, hest,
       SUM(tweight) wstarter
       --ROUND(SUM(tweight*start), 4) wstarter
FROM antstarterwtimeweight40
GROUP BY dato, tidspunkt, travbane, lopnr, hest
ORDER BY dato DESC, travbane DESC, lopnr ASC, hest ASC;
...
SELECT d.dato, d.tidspunkt, d.travbane, d.lopnr, d.hest, d.snr, d.rase, d.hdist, d.km okm,
       l.dato ldato, l.travbane lbane, l.lopnr lnr,
       CASE WHEN ((d.dato - l.dato)::NUMERIC / 365) <= 1 THEN 0.4 ELSE ROUND(0.4^((d.dato - l.dato)::NUMERIC / 365), 6) END tweight,
       l.km, l.premie
FROM dnf d
...
CREATE MATERIALIZED VIEW weightedkm40 AS
...
INNER JOIN weightedstarter40 s ON
...
drop materialized view h2hv40, wkmpluswg40, weightedkm40, kmtimeweight40, weightedstarter40, antstarterwtimeweight40, weightedg40, gwtimeweight40;
