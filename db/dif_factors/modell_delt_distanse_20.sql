-- bruker 0.2 som vekt
CREATE MATERIALIZED VIEW gwtimeweight20 AS
SELECT
  s.dato, s.tidspunkt, s.travbane, s.lopnr, s.hest,
  g.dato ldato, g.travbane lbane, g.lopnr lnr,
  CASE WHEN ((s.dato - g.dato)::NUMERIC / 365) <= 1 THEN 0.2 ELSE ROUND(0.2^((s.dato - g.dato)::NUMERIC / 365), 6) END tweight,
...
CREATE MATERIALIZED VIEW weightedg20 AS
...
CREATE MATERIALIZED VIEW antstarterwtimeweight20 AS
SELECT f.dato, f.tidspunkt, f.travbane, f.lopnr, f.hest,
       l.dato ldato, l.travbane lbane, l.lopnr lnr,
       CASE WHEN ((f.dato - l.dato)::NUMERIC / 365) <= 1 THEN 0.2 ELSE ROUND(0.2^((f.dato - l.dato)::NUMERIC / 365), 6) END tweight,
       1 AS start
FROM starter f
...
SELECT d.dato, d.tidspunkt, d.travbane, d.lopnr, d.hest, d.snr, d.rase, d.hdist, d.km okm,
       l.dato ldato, l.travbane lbane, l.lopnr lnr,
       CASE WHEN ((d.dato - l.dato)::NUMERIC / 365) <= 1 THEN 0.2 ELSE ROUND(0.2^((d.dato - l.dato)::NUMERIC / 365), 6) END tweight,
       l.km, l.premie
FROM dnf d
...
CREATE MATERIALIZED VIEW weightedkm20 AS
...
-- for csv
CREATE MATERIALIZED VIEW h2hv20 AS
...
drop materialized view h2hv20, wkmpluswg20, weightedkm20, kmtimeweight20, weightedstarter20, antstarterwtimeweight20, weightedg20, gwtimeweight20;
