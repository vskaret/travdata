...
-- kopiere til databasen:
COPY dnf FROM '/home/postgres/midl/ny_dnf.csv' DELIMITER ';' CSV;
COPY finished FROM '/home/postgres/midl/ny_finished.csv' DELIMITER ';' CSV;
-- kopiere fra databasen og sortere på dato osv:
SELECT * FROM dnf ORDER BY dato DESC, travbane DESC, lopnr DESC;
SELECT * FROM finished ORDER BY dato DESC, travbane DESC, lopnr DESC;
-- COPY (SELECT * FROM <tablename>) TO '<path>' WITH CSV DELIMITER '<delim>';
COPY (SELECT * FROM dnf ORDER BY dato DESC, travbane DESC, lopnr DESC) TO '/home/postgres/midl/sortert_dnf.csv' DELIMITER ';';
COPY (SELECT * FROM finished ORDER BY dato DESC, travbane DESC, lopnr DESC) TO '/home/postgres/midl/sortert_finished.csv' DELIMITER ';';

-- Kopier til ny databasen
COPY lop FROM '/var/lib/pgsql/csv/lop.csv' DELIMITER ';' CSV;
COPY dnf FROM '/var/lib/pgsql/csv/dnf.csv' DELIMITER ';' CSV;
COPY finished FROM '/var/lib/pgsql/csv/finished.csv' DELIMITER ';' CSV;


COPY (SELECT * FROM r3l) TO '/var/lib/pgsql/midl/r3l.csv' DELIMITER ';';
COPY (SELECT * FROM r5l) TO '/var/lib/pgsql/midl/r5l.csv' DELIMITER ';';
COPY (SELECT * FROM r3l WHERE rase = 'Varmblods') TO '/var/lib/pgsql/midl/r3lv.csv' DELIMITER ';';
COPY (SELECT * FROM r3l WHERE rase = 'Kaldblods') TO '/var/lib/pgsql/midl/r3lk.csv' DELIMITER ';';
COPY (SELECT * FROM r5l WHERE rase = 'Varmblods') TO '/var/lib/pgsql/midl/r5lv.csv' DELIMITER ';';
COPY (SELECT * FROM r5l WHERE rase = 'Kaldblods') TO '/var/lib/pgsql/midl/r5lk.csv' DELIMITER ';';
